...
export default function PackageDetail({ pkg, bankInfo, onBack, onRent, isRented, onAddReview, onDelete, subscription }: PackageDetailProps) {
  ...
  const calculatedWinRate = pkg.reviews.length > 0 
    ? `${(60 + (averageRating * 7)).toFixed(1)}%`
    : pkg.winRate || pkg.stats?.monthlyWinRate || '85.5%';
  ...
  const allGalleryImages = [
    ...((pkg.demoImageUrls && pkg.demoImageUrls.length > 0) ? pkg.demoImageUrls : (pkg.demoImageUrl ? [pkg.demoImageUrl] : [])),
    ...(pkg.backtestImageUrls || [])
  ];

  const demoImagesCount = (pkg.demoImageUrls && pkg.demoImageUrls.length > 0) ? pkg.demoImageUrls.length : (pkg.demoImageUrl ? 1 : 0);
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div className="space-y-8">
            ...
            >
              {(pkg.videoUrl || pkg.demoImageUrl || (pkg.demoImageUrls && pkg.demoImageUrls.length > 0)) ? (
                <>
                  {pkg.videoUrl ? (
                    pkg.videoUrl.startsWith('blob:') || pkg.videoUrl.includes('data:video') ? (
                      <video 
                        src={pkg.videoUrl} 
                    ...
                    <div className="w-full h-full relative group/slides">
                      <img 
                        src={pkg.demoImageUrls?.[0] || pkg.demoImageUrl} 
                        className="w-full h-full object-cover"
                      />
                      {pkg.demoImageUrls && pkg.demoImageUrls.length > 1 && (
                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 z-10">
                          {pkg.demoImageUrls.map((_, idx) => (
                            <div key={idx} className="w-1.5 h-1.5 rounded-full bg-white/50" />
                          ))}
                        </div>
                      )}
                      <div className="absolute inset-0 bg-black/20 opacity-0 group-hover/slides:opacity-100 transition-opacity flex items-center justify-center">
                        <p className="text-[10px] font-black text-white uppercase tracking-widest bg-black/60 px-4 py-2 rounded-full border border-white/10">
                          {pkg.demoImageUrls ? `${pkg.demoImageUrls.length} Screenshots` : '1 Screenshot'}
                        </p>
                      </div>
                    </div>
                  )}
                ...
                <div className="w-full h-full flex flex-col items-center justify-center text-gray-600">
                  ...
                  <p className="font-bold uppercase tracking-widest text-xs">Preview Image</p>
                </div>
              )}
            </motion.div>
            ...
            <div className="p-6 sm:p-8 bg-black/40 rounded-[32px] border border-white/5 shadow-2xl relative overflow-hidden group">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mb-8">
                <div className="bg-white/5 p-5 sm:p-6 rounded-2xl border border-white/5">
                  <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Avg. Target Win Rate</p>
                  <p className="text-2xl sm:text-3xl font-black text-white">{calculatedWinRate}</p>
                  <div className="mt-2 h-1 bg-white/5 rounded-full overflow-hidden">
                    <div className="h-full bg-green-500 transition-all duration-1000" style={{ width: calculatedWinRate }} />
                  </div>
                </div>
                <div className="bg-white/5 p-5 sm:p-6 rounded-2xl border border-white/5">
            ...
            <div className="p-8 bg-yellow-500/5 rounded-3xl border border-yellow-500/20">
              ...
              {(pkg.currentUsers !== undefined || pkg.winRate || pkg.usersLastMonth !== undefined) && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8 pt-6 border-t border-yellow-500/10">
                  ...
                  <div>
                    <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">อัตราชนะ (อ้างอิงจากรีวิว)</p>
                    <p className="text-xl font-black text-green-500">{calculatedWinRate}</p>
                  </div>
                </div>
              )}
            </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                  {pkg.backtestImageUrls.map((url, i) => (
                    <motion.div 
                      ...
                      whileTap={{ scale: 0.95 }}
                      onClick={() => setLightbox({ isOpen: true, index: i + demoImagesCount })}

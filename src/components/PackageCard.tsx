...
export default function PackageCard({ pkg, onSelect, onDelete, isPopular, isExpired }: PackageCardProps) {
  ...
  // Dynamic Win Rate based on reviews performance/satisfaction
  const calculatedWinRate = pkg.reviews.length > 0 
    ? `${(60 + (averageRating * 7)).toFixed(1)}%`
    : pkg.winRate || pkg.stats?.dailyWinRate || '85.5%'; // Fallback for new EAs
    ...
    >
      ...
      <div className="mb-6 flex-grow">
        ...
        <div className="mt-4 flex items-center justify-between">
          <div className="flex flex-col">
            <span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Win Rate (Review Based)</span>
            <span className="text-sm font-black text-green-500">{calculatedWinRate}</span>
          </div>
        </div>
